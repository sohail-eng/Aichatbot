...
class FileAnalyzer:
    ...
    def analyze_question_with_files(
        self, question: str, attached_files: List[Dict]
    ) -> Dict:
        """
        ...
        try:
            logger.info(
                f"Analyzing question: '{question}' with {len(attached_files)} files"
            )
            ...
            for file_info in attached_files:
                ...
                analysis = self._analyze_file(
                    file_path, file_info, search_terms, question
                )
    ...
    async def analyze_question_with_files_async(
        self, question: str, attached_files: List[Dict]
    ) -> Dict:
        """
        ...
        try:
            logger.info(
                f"Analyzing question (async): '{question}' with {len(attached_files)} files"
            )
    ...
    def _extract_search_terms(self, question: str) -> List[str]:
        ...
        # Add semantic variations for common data analysis terms
        semantic_variations = []

        # Status variations
        if any(
            term in question_lower
            for term in ["status", "state", "condition", "health"]
        ):
            semantic_variations.extend(["status", "state", "condition", "health"])

        # Time/date variations
        if any(
            term in question_lower
            for term in ["when", "date", "time", "period", "duration"]
        ):
            semantic_variations.extend(["date", "time", "timestamp", "period", "duration"])

        # Quantity variations
        if any(
            term in question_lower
            for term in ["count", "total", "sum", "average", "mean"]
        ):
            semantic_variations.extend(["count", "total", "sum", "average", "mean"])

        # Location/position variations
        if any(
            term in question_lower
            for term in ["where", "location", "place", "position"]
        ):
            semantic_variations.extend(["location", "place", "position", "address"])
    ...
    async def _resolve_file_path_async(self, file_info: Dict) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                if file_id and str(file_id).isdigit():
                    try:
                        file_obj = await sync_to_async(UploadedFile.objects.get)(
                            id=file_id
                        )
                        full_path = Path(settings.MEDIA_ROOT) / file_obj.file_path
                        if full_path.exists():
                ...
                if file_name:
                    file_obj = await sync_to_async(
                        UploadedFile.objects.filter(file_name=file_name).first
                    )()
                    if file_obj:
                        full_path = Path(settings.MEDIA_ROOT) / file_obj.file_path
                        if full_path.exists():
    ...
    def _analyze_file(
        self, file_path: str, file_info: Dict, search_terms: List[str], question: str
    ) -> Dict:
    ...
    def _analyze_csv(
        self, file_path: str, search_terms: List[str], question: str
    ) -> Dict:
        """Analyze CSV file"""
        try:
    ...
    def _apply_intelligent_filter(
        self, df: pd.DataFrame, question: str, search_terms: List[str]
    ) -> pd.DataFrame:
        """Apply intelligent filtering based on question context"""
        try:
            question_lower = question.lower()

            # State/condition based filtering
            state_columns = [
                col
                for col in df.columns
                if any(
            ...
            if state_columns:
                ...
                # Check if any search terms match state values
                patterns = [term.lower() for term in search_terms]
                mask = (
                    df[state_col]
                    .astype(str)
                    .str.lower()
                    .str.contains("|".join(patterns), na=False)
                )
                if mask.any():
                    return df[mask]

            # Entity-based filtering (which/what items)
            if "which" in question_lower or "what" in question_lower:
                ...
                id_columns = [
                    ...
                    if any(
                        word in str(col).lower()
                        for word in ["name", "id", "title", "identifier", "key"]
                    )
                ]
                if id_columns:
                    # Return data with non-null identifying columns
                    return df.dropna(subset=[id_columns[0]])
            ...
            if "how many" in question_lower:
                ...
                if group_columns:
                    grouped = (
                        df.groupby(group_columns[0]).size().reset_index(name="count")
                    )
                    return grouped
    ...
    def _analyze_csv_dataframe(
        self, df: pd.DataFrame, search_terms: List[str], question: str
    ) -> Dict:
    ...
    def _analyze_json(
        ...
        try:
            ...
            summary = (
                f"JSON file with {len(matches)} matches"
                if matches
                else "JSON file with no matches"
            )
            ...
            for term in search_terms:
                ...
                if term_lower in content_lower:
                    ...
                    while True:
                        ...
                        matches.append(
                            {"search_term": term, "position": pos, "context": context}
                        )
                        relevance_score += 1
                        start = pos + 1

            summary = (
                f"Text file with {len(matches)} matches"
                if matches
                else "Text file with no matches"
            )
    ...
    def _generate_csv_summary(self, found_data: Dict, relevance_score: int) -> str:
        ...
        if relevance_score == 0:
            return (
                f"CSV file with {found_data['total_rows']} rows, no relevant data found"
            )
        ...
        if found_data["matching_columns"]:
            summary_parts.append(
                f"relevant columns: {', '.join(found_data['matching_columns'])}"
            )
        ...
        for i, data in enumerate(all_found_data, 1):
            ...
            if isinstance(file_data, dict):
                if "matching_columns" in file_data and file_data["matching_columns"]:
                    analysis_parts.append(
                        f"   MATCHING COLUMNS: {', '.join(file_data['matching_columns'])}"
                    )

                if "filtered_data" in file_data and file_data["filtered_data"]:
                    analysis_parts.append(
                        f"   FILTERED RESULTS: {len(file_data['filtered_data'])} rows"
                    )
                    analysis_parts.append(
                        f"   SAMPLE DATA: {json.dumps(file_data['filtered_data'][:3], indent=2)}"
                    )
                    ...
                    for match in file_data["matching_data"][:2]:  # Show top 2 matches
                        analysis_parts.append(
                            f"     - {match['search_term']} in column '{match['column']}': {match['matches']} matches"
                        )
