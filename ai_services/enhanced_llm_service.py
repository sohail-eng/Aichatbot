...
class EnhancedLLMService:
    ...
    async def process_question_with_files(
        ...
        try:
            logger.info(
                f"Processing question with {len(attached_files)} attached files"
            )
            ...
            rag_context = self.rag_service.get_context_for_question(
                session_id, question, file_names
            )

            # Step 2: Generate LLM response based on analysis and RAG context
            llm_response = await self._generate_llm_response(
                question, file_analysis, rag_context
            )

            # Step 3: Compile final response
            final_response = self._compile_final_response(
                file_analysis, llm_response, rag_context
            )
            ...
            return {
                ...
                "metadata": {
                    ...
                    "rag_enhanced": rag_context.get("success", False)
                    if rag_context
                    else False,
    ...
    async def _generate_llm_response(
        self, question: str, file_analysis: Dict, rag_context: Dict = None
    ) -> str:
        """
        ...
        try:
            ...
            # Note: search_keywords might be used in future improvements
            # for now, we'll use RAG context directly
            ...
            if rag_context and rag_context.get("success"):
                ...
                return await self._generate_data_driven_response(
    ...
    async def _generate_no_data_response(
        self, question: str, file_analysis: Dict
    ) -> str:
        """
    ...
    async def _generate_data_driven_response(
        self, question: str, file_analysis: Dict
    ) -> str:
        """
    ...
    async def _generate_rag_enhanced_response(
        self, question: str, file_analysis: Dict, rag_context: Dict
    ) -> str:
        """
...
"""
    ...
    def _compile_final_response(
        self, file_analysis: Dict, llm_response: str, rag_context: Dict = None
    ) -> str:
        """
        ...
        try:
            ...
            analysis_summary += (
                f"• Search keywords: {', '.join(file_analysis['search_keywords'])}\n"
            )

            if found_data:
                analysis_summary += "• Relevant data found: Yes\n"
                analysis_summary += f"• Data instances: {len(found_data)}\n"
            else:
                analysis_summary += "• Relevant data found: No\n"

            # Add RAG information if available
            if rag_context and rag_context.get("success"):
                analysis_summary += "• RAG-enhanced: Yes\n"
                analysis_summary += (
                    f"• Context sources: {rag_context.get('total_sources', 0)}\n"
                )
