...
class ChatProcessor:
    ...
    async def process_message(
        ...
        try:
            ...
            if attached_files:
                logger.info(
                    f"Processing message with {len(attached_files)} attached files"
                )
                ...
                if intent["type"] == "database_query":
                    response = await self._process_database_query(
                        session, message_content, context, intent
                    )
                elif intent["type"] == "file_analysis":
                    response = await self._process_file_analysis(
                        session, message_content, context, intent
                    )
                elif intent["type"] == "general_chat":
                    response = await self._process_general_chat(
                        session, message_content, context
                    )
                else:
                    response = await self._process_general_chat(
                        session, message_content, context
    ...
    async def process_file_upload(
        self, session_id: str, uploaded_file, user_question: str = ""
    ) -> Dict:
        ...
        try:
            ...
            processed_file = self.file_service.process_file(
                file_info["full_path"], file_info["file_type"]
            )
            ...
            if rag_result["success"]:
                logger.info(
                    f"File processed for RAG: {rag_result['chunks_created']} chunks created"
                )
            else:
                logger.warning(
                    f"RAG processing failed: {rag_result.get('error', 'Unknown error')}"
                )
            ...
            try:
                ...
                analysis_response = self._generate_fallback_analysis(
                    processed_file, file_info, user_question
                )
...
Files found:"""
    ...
    async def process_database_connection(
        self, session_id: str, connection_config: Dict
    ) -> Dict:
        ...
        try:
            ...
            await self._create_message(
                session,
                "system",
                f"Connected to database: {connection_config['server']}/{connection_config['database']}",
    ...
    def _get_or_create_session(self, session_id: str) -> ChatSession:
        ...
        session, created = ChatSession.objects.get_or_create(
            session_id=session_id, defaults={"created_at": timezone.now()}
        )
        return session

    @database_sync_to_async
    def _get_conversation_context(
        self, session: ChatSession, limit: int = 10
    ) -> List[Dict]:
    ...
    async def _process_database_query(
        self, session: ChatSession, message: str, context: List[Dict], intent: Dict
    ) -> Dict:
        ...
        if message.strip().upper().startswith("SELECT"):
            ...
            result = self.database_service.execute_query(
                connection_id, message, session.session_id
            )
            ...
            if schema:
                generated_query = await self.llama_service.generate_sql_query(
                    message, schema
                )
    ...
    async def _process_general_chat(
        self, session: ChatSession, message: str, context: List[Dict]
    ) -> Dict:
        ...
        try:
            ...
            return {"content": response, "metadata": {"chat_type": "general"}}
        except Exception as e:
    ...
    def _generate_fallback_analysis(
        self, processed_file: Dict, file_info: Dict, user_question: str = ""
    ) -> str:
        ...
        if file_type == "csv":
            ...
            if user_question:
                ...
                response += (
                    "• The file contains structured data with multiple columns\n"
                )
                response += (
                    "• You can ask questions about specific columns or data patterns\n"
                )
                response += (
                    "• I can help you understand the data structure and content\n\n"
                )
    ...
    async def _handle_row_question(self, message: str, file_obj) -> Dict:
        ...
        try:
            ...
            processed_file = self.file_service.process_file(
                full_file_path, file_obj.file_type
            )
            if not processed_file["success"]:
                return {
    ...
    async def _handle_general_file_question(self, message: str, file_obj) -> Dict:
        ...
        try:
            ...
            try:
                ...
                response = self._generate_fallback_analysis(
                    processed_file,
                    {
    ...
    def _get_session_files_async(self, session):
        ...
        return list(
            UploadedFile.objects.filter(session=session).order_by("-upload_timestamp")
        )
