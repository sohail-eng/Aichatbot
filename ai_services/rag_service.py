...
class RAGService:
    ...
    def _split_text_into_chunks(self, text: str) -> List[str]:
        ...
        while start < len(text):
            ...
            if end < len(text):
                ...
                if (
                    break_point > start + self.chunk_size * 0.5
                ):  # At least 50% of chunk size
    ...
    def _create_document_chunks(
        self, content_chunks: List[Dict], file_name: str, file_type: str
    ) -> List[DocumentChunk]:
    ...
    def get_context_for_question(
        self, session_id: str, question: str, file_names: List[str]
    ) -> Dict:
        """
        ...
        try:
            ...
            for file_name in file_names:
                ...
                file_results = self.search_relevant_chunks(
                    session_id, question, [file_name], n_results=3
                )
            ...
            for file_name in file_names:
                if file_name in file_results_map and file_results_map[file_name]:
                    best_result = max(
                        file_results_map[file_name], key=lambda x: x.score
                    )
                ...
                sources.append(
                    {
                        ...
                        "chunk_type": result.chunk.metadata.get(
                            "chunk_type", "unknown"
                        ),
            ...
            logger.info(
                f"Context assembled from {len(sources)} sources in files: {file_names}"
            )
            ...
            return {
                ...
                "average_relevance": sum(r.score for r in final_results)
                / len(final_results),
            }
    ...
    def clear_file_chunks(self, session_id: str, file_name: str):
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Cleared chunks for file: {file_name} in session {session_id}"
                )
            else:
